...
import { AntiAliasingType, Circle, InfiniteCanvas } from "@/components/lesson-two";
import {
	SelectContent,
	SelectControl,
	SelectItem,
...
export const Route = createFileRoute("/lessons/two")({ component: App });
...
function Canvas(props: {
	...
	onMount(async () => {
		...
		const circle = new Circle({
			antiAliasingType: AntiAliasingType.FWIDTH,
			cx: props.size.width / 2 - 125,
			cy: props.size.height / 2 - 125,
			fill: "white",
			r: 100,
		});
...
function App() {
	const collection = createListCollection<Required<InfiniteCanvasConfig["renderer"]>>({
		items: ["webgl", "webgpu"],
	});
	const [renderer, setRenderer] =
		createSignal<Exclude<InfiniteCanvasConfig["renderer"], undefined>>("webgl");
		...
		<div class="flex min-h-screen flex-col items-center justify-center gap-2">
			...
			<ClientOnly fallback={<h1>Loading...</h1>}>
				<Show when={renderer() === "webgl"}>
					<Canvas renderer="webgl" size={size()} />
				</Show>
				<Show when={renderer() === "webgpu"}>
					<Canvas renderer="webgpu" size={size()} />
				</Show>
			</ClientOnly>
		</div>
	);
